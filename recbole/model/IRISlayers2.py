...
class IRISMultiHeadAttention(nn.Module):
    """
    IRIS Multi-head Self-attention layers, a attention score dropout layer is introduced.
    ...
    def __init__(self, n_heads, hidden_size, hidden_dropout_prob, attn_dropout_prob, layer_norm_eps, fusion_type, max_len):
        ...
        if self.fusion_type == 'concat':
            self.fusion_layer = nn.Linear(self.max_len*(2), self.max_len)#max_len; |{id, position}| = 2
    ...
    def forward(self, input_tensor, position_embedding, attention_mask):
        ...
        if self.fusion_type == 'sum':
            ...
            attention_scores = torch.cat([item_attention_scores, pos_scores], dim=-1)
...
class FeedForward(nn.Module):
    ...
    def gelu(self, x):
        ...
        For information: OpenAI GPT's gelu is slightly IRISferent (and gives slightly IRISferent results)::
