...
######attention fusion을 완전히 제거한 후 value에만 곱하기######
class IRISMultiHeadAttention(nn.Module):
    ...
    def __init__(self, n_heads, hidden_size, attribute_hidden_size, hidden_dropout_prob, attn_dropout_prob, layer_norm_eps, fusion_type, max_len):
        ...
        self.attribute_attention_head_size = int(attribute_hidden_size[0] / n_heads)
        self.attribute_all_head_size = self.num_attention_heads * self.attribute_attention_head_size
        ...
        #attribute
        self.query_layer = nn.Linear(attribute_hidden_size[0], self.attribute_all_head_size)
        self.key_layer = nn.Linear(attribute_hidden_size[0], self.attribute_all_head_size)

        if self.fusion_type == 'concat':
            self.fusion_layer = nn.Linear(self.max_len*(2), self.max_len)#max_len; |{att, position}| = 2
    ...
    def forward(self, input_tensor,attribute_tensor, position_embedding, attention_mask):
        ...
        #attribute_attenttion score
        attribute_attention_table = []
        attribute_query = self.query_layer
        attribute_key = self.key_layer        
        attribute_tensor = attribute_tensor.squeeze(-2)

        attribute_query_layer = self.transpose_for_scores_attribute(attribute_query(attribute_tensor))
        attribute_key_layer = self.transpose_for_scores_attribute(attribute_key(attribute_tensor))  
        ...
        # table_shape = attribute_attention_scores.shape
        attention_size = table_shape[-1]

        if self.fusion_type == 'sum':
            attention_scores = torch.sum(attribute_attention_table, dim=-2)
            attention_scores = attribute_attention_table +  pos_scores
        elif self.fusion_type == 'concat':
            attention_scores = attribute_attention_table.view(table_shape[:-2] + (attention_size,))
            ...
            attention_scores = torch.cat(
                [attribute_attention_table, pos_scores.unsqueeze(-2)], dim=-2)
...
class IRISTransformerLayer(nn.Module):
    ...
    def __init__(
        self, n_heads, hidden_size,attribute_hidden_size, intermediate_size, hidden_dropout_prob, attn_dropout_prob, hidden_act,
        ...
        self.multi_head_attention = IRISMultiHeadAttention(
            n_heads, hidden_size, attribute_hidden_size, hidden_dropout_prob, attn_dropout_prob, layer_norm_eps, fusion_type, max_len,
        )
        self.feed_forward = FeedForward(hidden_size, intermediate_size, hidden_dropout_prob, hidden_act, layer_norm_eps)
...
class IRISTransformerEncoder(nn.Module):
    ...
    def __init__(
        ...
        layer = IRISTransformerLayer(
            n_heads, hidden_size,attribute_hidden_size, inner_size, hidden_dropout_prob, attn_dropout_prob, hidden_act, layer_norm_eps, fusion_type, max_len
        )
        self.layer = nn.ModuleList([copy.deepcopy(layer) for _ in range(n_layers)])
