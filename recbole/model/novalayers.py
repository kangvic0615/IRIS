...
class NOVAMultiHeadAttention(nn.Module):
    """
    NOVA Multi-head Self-attention layers, a attention score dropout layer is introduced.
    ...
    def __init__(self, n_heads, hidden_size,attribute_hidden_size, feat_num, hidden_dropout_prob, attn_dropout_prob, layer_norm_eps, fusion_type, max_len):
        ...
        if self.fusion_type == 'concat':
            self.fusion_layer = nn.Linear(hidden_size*(1+self.feat_num), hidden_size)
    ...
    def forward(self, input_tensor, attribute_table, attention_mask):
        ...
        if self.fusion_type == 'sum':
            fusion_value = torch.sum(attribute_table, dim=-2)
            fusion_value = fusion_value + input_tensor + position

        elif self.fusion_type == 'concat':
            fusion_value = attribute_table.view(table_shape[:-2] + (feat_atten_num * attention_size,))
            fusion_value = torch.cat([fusion_value, input_tensor], dim=-1)
            fusion_value = self.fusion_layer(fusion_value)

        elif self.fusion_type == 'gate':
            fusion_value = torch.cat(
                [attribute_table, input_tensor.unsqueeze(-2)], dim=-2)
            fusion_value,_ = self.fusion_layer(fusion_value)

        mixed_tensor = fusion_value.squeeze(-2)
        mixed_query_layer = self.transpose_for_scores(self.query(mixed_tensor))
        mixed_key_layer = self.transpose_for_scores(self.key(mixed_tensor))

        mixed_attention_scores = torch.matmul(mixed_query_layer, mixed_key_layer.transpose(-1, -2))
        mixed_attention_scores = mixed_attention_scores / math.sqrt(self.attention_head_size)
        # Apply the attention mask is (precomputed for all layers in BertModel forward() function)
        # [batch_size heads seq_len seq_len] scores
        # [batch_size 1 1 seq_len]

        mixed_attention_scores = mixed_attention_scores + attention_mask

        # Normalize the attention scores to probabilities.
        attention_probs = nn.Softmax(dim=-1)(mixed_attention_scores)
...
class NOVATransformerEncoder(nn.Module):
    ...
    def forward(self, hidden_states,attribute_hidden_states, attention_mask, output_all_encoded_layers=True):
        """
        Args:
            hidden_states (torch.Tensor): the input of the TransformerEncoder
            attention_mask (torch.Tensor): the attention mask for the input hidden_states
