...
class IRIS2(SequentialRecommender):
    """
    ...
    def __init__(self, config, dataset):
        super(IRIS2, self).__init__(config, dataset)
...
########################################################################################################################
    ...
    def plot_attention_matrix(self, attention_matrix, title):
        fig, ax = plt.subplots(figsize=(4, 3))  # 이미지 크기를 더욱 축소합니다.
        im = ax.imshow(attention_matrix, cmap='Blues', aspect='auto', vmin=0, vmax=1, interpolation='nearest')
        ax.set_title(title)
    ...
    def full_sort_predict(self, interaction):
        ...
        ###plot###
        # self.plot_attention_matrix(seq_output.cpu(), 'Item Combined Attention Matrix')
        # for i in range(len(seq_outputs)):
        #     self.plot_attention_matrix(seq_outputs[i].cpu(), f'Item x Attribute {i+1} Attention Matrix {i+1}')
        return scores