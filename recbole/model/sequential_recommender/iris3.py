...
class IRIS3(SequentialRecommender):
    """
    ...
    def __init__(self, config, dataset):
        super(IRIS3, self).__init__(config, dataset)
...
########################################################################################################################
    ...
    def calculate_loss(self, interaction):
        ...
        if self.loss_type == 'BPR':
            ...
            # #logits -> MF layer(y^)를 형성해서 interaction score를 계산한 것
            # #loss에서 이를 crossentropy loss로 계산 (ylog(y^))
            test_item_emb = self.item_embedding.weight
            total_loss = 0
    ...
    def plot_attention_matrix(self, attention_matrix, title):
        ...
        plt.show()
         ###plot###
        # self.plot_attention_matrix(item_attention_scores[0].detach().cpu(), 'Item Attention Matrix')
        # for i in range(len(attention_scores)):
        #     self.plot_attention_matrix(attention_scores[i][0].detach().cpu(), f'Attribute Attention Matrix {i}')
