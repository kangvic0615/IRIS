...
class IRIS4(SequentialRecommender):
    """
    ...
    def __init__(self, config, dataset):
        super(IRIS4, self).__init__(config, dataset)
...
########################################################################################################################
    ...
    def calculate_loss(self, interaction):
        ...
        if self.loss_type == 'BPR':
            ...
            test_item_emb = self.item_embedding.weight
            total_loss = self.loss_fct(torch.matmul(seq_output, test_item_emb.transpose(0,1)), pos_items)
