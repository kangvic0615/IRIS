...
from recbole.model.novalayers import FeatureSeqEmbLayer,NOVATransformerEncoder
...
class NOVA(SequentialRecommender):
    """
    NOVA-SR moves the side information from the input to the attention layer and decouples the attention calculation of
    ...
    def __init__(self, config, dataset):
        ...
        self.feature_embed_layer_list = nn.ModuleList(
            [copy.deepcopy(FeatureSeqEmbLayer(dataset, self.attribute_hidden_size[_], [self.selected_features[_]], self.pooling_mode, self.device)) for _
             in range(len(self.selected_features))]) ## 특성별 임베딩 레이어 리스트 형성 [[brand], [category], ...]
    ...
    def forward(self, item_seq, item_seq_len):
        ...
        # concat the sparse embedding and float embedding
            if sparse_embedding is not None:
                ...
                # print("\nitem_id_emb - sparse_embedding:\n", sparse_embedding.size())
            if dense_embedding is not None:
                feature_table.append(dense_embedding)
                # print("item_id_emb - dense_embedding:", dense_embedding.size())
        ...
        trm_output = self.trm_encoder(input_emb, feature_emb, extended_attention_mask, output_all_encoded_layers=True)
        output = trm_output[-1]
    ...
    def calculate_loss(self, interaction):
        ...
        if self.loss_type == 'BPR':
            ...
            # print("pos_items size:", pos_items.size())
