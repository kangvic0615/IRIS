...
class LinearFusionModel(nn.Module):
    def __init__(self, input_size, output_size, dropout=0.2, activation='gelu', bn=True):
        ...
        self.fc4 = nn.Linear(64, output_size, bias=True)
    ...
    def forward(self, input_feature):
        x = input_feature
        x1 = fn.relu(self.fc1(x))
        x2 = self.dropout(fn.relu(self.fc2(x1)))
        x3 = fn.relu(self.fc3(x2))
        
        # Attention mechanism
        attn_weights = fn.softmax(x3, dim=1)
        attended_x2 = torch.sum(attn_weights.unsqueeze(2) * x2.unsqueeze(1), dim=1)

        if self.use_bn:
            attended_x2 = self.bn2(attended_x2)

        output = fn.relu(self.fc4(attended_x2))

        # x = input_feature
        # x = fn.gelu(self.fc1(x))
        # x = fn.gelu(self.fc2(x))
        # x = fn.gelu(self.fc3(x))
        # if self.use_bn:
        #     x = self.bn1(x)
        # output = fn.gelu(self.fc4(x))
        # print("\n_________________output________________", output)
        return output
        
# class LinearFusionModel(nn.Module):
#     def __init__(self, input_size, output_size, dropout=0.2, activation='relu', bn=False):
#         super(LinearFusionModel, self).__init__()
#         # self.embedding_layer = FMEmbedding(field_dims=[input_size], offsets=[0], embed_dim=output_size)
#         self.mlp_layers = MLPLayers(layers=[output_size], dropout=dropout, activation=activation, bn=bn)

#     def forward(self, input_feature):
#         # # Assuming input_feature is a tensor of shape (batch_size, num_features, feature_dim)
#         # # where num_features is the number of input feature vectors (in this case, 3)
        
#         # # Reshape input_feature to (batch_size * num_features, feature_dim)
#         # input_feature_reshaped = input_feature.view(-1, input_feature.size(-1))
        
#         # # Apply FMEmbedding to get embeddings
#         # embedded_input = self.embedding_layer(input_feature_reshaped)
        
#         # # Reshape embedded_input back to (batch_size, num_features, output_size)
#         # embedded_input = embedded_input.view(input_feature.size(0), input_feature.size(1), -1)
        
#         # Apply MLPLayers
#         output = self.mlp_layers(input_feature.squeeze(dim=1))
...
class AttentionFusion(nn.Module):
    def __init__(self, input_size):
        ...
        self.attention_weights = nn.Parameter(torch.rand(input_size, 1))
    ...
    def forward(self, x):
        ...
        # 어텐션 가중합
        fused_embedding = torch.sum(x * attention_weights, dim=1)

        return fused_embeddin
...
class DIFMultiHeadAttention(nn.Module):
    ...
    def __init__(self, n_heads, hidden_size,attribute_hidden_size, feat_num, hidden_dropout_prob, attn_dropout_prob, layer_norm_eps, fusion_type, max_len):
        ...
        self.feat_num = 1 # always there is a feature
...
#     def forward(self, hidden_states_dict, attribute_hidden_states, position_embedding, attention_mask, output_all_encoded_layers=True):
#         # Use separate hidden states for each feature
#         all_encoder_layers = []
#         for layer_module in self.layer:
#             # Assuming 'category', 'brand', 'title' are the features
#             hidden_states = torch.cat([hidden_states_dict[feature] for feature in ['category', 'brand', 'title']], dim=-1)
